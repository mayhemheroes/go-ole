#!/usr/bin/env bash
#
# go-ole/mayhem/test.sh — RUN go-ole/go-ole's OWN Go test suite (`go test ./...`) and emit a
# CTRF summary. exit 0 iff no test failed.
#
# PATCH-grade oracle: go-ole's suite is a REAL known-answer suite — guid_test.go/safearray*_test.go assert GUID
# parse/format round-trips and SafeArray conversions against expected values (known-answer
# tests). They assert BEHAVIOUR, not "exits 0", so a no-op /
# `return` patch that breaks GUID parsing FAILS this oracle. This script only RUNS the
# suite (the project's own normal-flags suite — no sanitizer/fuzz build here).
#
...
# /mayhem/fuzz-guid (dynamically linked, ASan+libFuzzer) against a known corpus entry and
# asserts specific libFuzzer output strings ("Executed ... in"). A no-op / exit(0) PATCH to
# go-ole leaves fuzz-guid intact (it IS the compiled Go code), so it still emits the
# expected output. When the SABOTAGE MECHANISM (LD_PRELOAD _exit(0)) neuters fuzz-guid itself,
# fuzz-guid exits silently and the grep fails — proving the oracle detects sabotage (not
# reward-hackable).
set -uo pipefail
...
# ── Behavioral probe via the dynamically-linked fuzz-guid binary (anti-reward-hacking, §6.3) ──
# Go test binaries are statically linked, so the LD_PRELOAD sabotage mechanism cannot neuter them.
# /mayhem/fuzz-guid IS dynamically linked (built with clang+ASan). Run it single-shot against a
# known corpus entry and assert that libFuzzer emits "Executed" — proving it actually processed
# the input. The sabotage LD_PRELOAD neuters fuzz-guid (not in /usr/bin etc.), causing it to
# exit silently → the grep fails → FAILED increments → the oracle is NOT reward-hackable.
PROBE_INPUT="$SRC/mayhem/fuzz-guid/testsuite/seed1"
if [ -x /mayhem/fuzz-guid ] && [ -f "$PROBE_INPUT" ]; then
  echo "=== behavioral probe: fuzz-guid single-shot on known corpus ==="
  PROBE_OUT=$(/mayhem/fuzz-guid "$PROBE_INPUT" 2>&1 || true)
  if echo "$PROBE_OUT" | grep -q "Executed"; then
    echo "PROBE PASS: fuzz-guid executed the corpus input (harness active)"
    PASSED=$(( PASSED + 1 ))
  else
    echo "PROBE FAIL: fuzz-guid produced no 'Executed' output (harness inactive or sabotaged)"
